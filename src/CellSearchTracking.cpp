...
string freq_formatter(
  ...
  stringstream temp;
  if (abs(freq)<998.0) {
    temp << setw(5) << setprecision(3) << freq << " Hz";
  } else if (abs(freq)<998000.0) {
    temp << setw(5) << setprecision(3) << freq/1e3 << " kHz";
...
int main(const int argc, char * const argv[]) 
{
  ...
  if ( dongle_used && freq_start!=9999e6) {
    ...
    #ifdef HAVE_HACKRF
    offset_tuning = false; // only useful for rtl-sdr
...
#define THRESH2_N_SIGMA 3
  ...
  // Loop for each center frequency. Here is important part of main loop
  //cout << "DPX CID A      fc   doppler-freq RX-power C nRB P  PR CrystalCorrectionFactor" << endl;
  cout << "---------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
  cout << "DPX CID A      fc   doppler-freq    pseudo-range   RX-power   Cyclix-Prefix   nRB   Phich-duration   Phitch-resource   CrystalCorrectionFactor    time-ms-since-epoch" << endl;
  cout << "---------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;

  for(int loop_cnt=0; loop_cnt<30; loop_cnt++) //added for test
  {
...
//      ++iterator;
  ...
  else 
  {
    ...
    //cout << "DPX CID A      fc   doppler-freq   RX-power   Cyclix-Prefix   nRB   Phich-duration   Phitch-resource   CrystalCorrectionFactor" << endl;
    ...
    while (it!=cells_final.end()) 
    {
      ...
      ss << " " << setw(15) << ((*it).freq_superfine * 299792458) / (2*(*it).fc_requested) ;
      ss << " " << setw(5+3) << setprecision(3) << db10((*it).pss_pow);
      ss << " " << setw(5+3+5) << (((*it).cp_type==cp_type_t::NORMAL)?"Normal":(((*it).cp_type==cp_type_t::UNKNOWN)?"Unkown":"Extend"));
      ss << " " << setw(3+7) << (*it).n_rb_dl;
      ss << " " << setw(8) << (((*it).phich_duration==phich_duration_t::NORMAL)?"N":(((*it).phich_duration==phich_duration_t::UNKNOWN)?"U":"E"));
      switch ((*it).phich_resource) 
      {
        case phich_resource_t::UNKNOWN: ss << setw(20) << " UNK"; break;
        case phich_resource_t::oneSixth: ss << setw(20) << " 1/6"; break;
        case phich_resource_t::half: ss << setw(20) << " 1/2"; break;
        case phich_resource_t::one: ss << setw(20) << " one"; break;
        case phich_resource_t::two: ss << setw(20) << " two"; break;
      }
...
//      }
      ss << setw(9) << " " << setprecision(20) << correction_new;
      ss << setw(8) << " " << millisec_since_epoch;
      cout << ss.str() << endl;
